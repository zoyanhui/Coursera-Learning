...
# Now implement the Bagging algorithm with N′=N 
# and couple it with your decision tree above to make a preliminary random forest GRS. 
# Produce T=300 trees with bagging. 
...
def CART_DT(mxy, d):
	...
	if impurity_left != 0:
		...
		print "mxy_left", mxy_left			
	dt_right = None
	numRight = 0
	if impurity_right != 0:
		if not is_x_same(mxy_right):
			dt_right = CART_DT(mxy_right, d)
			numRight = dt_right.get_internum()
		else:
			print "debug", mxy_right
			sum_y = 0
...
if __name__ == '__main__':
	...
	for r_time in range(repeat_times):
		...
		while t < T:
			...
			GtRF.append(gt)
			# print predict_err_mxy(gt, sample_mxy)
			ein = predict_err_mxy(gt, sample_mxy)			
			ein_gt_list.append(ein)		
		ein_grf = predict_err_grf(GtRF, mxy)		
		ein_grf_list.append(ein_grf)
		eout_grf_list.append(predict_err_grf(GtRF, mxy_test))

	# Q16 have problem, not right
	print "Q16, ave ein", sum(ein_gt_list) / len(ein_gt_list)
	# Q17
	print "Q17, ave Grf ein", sum(ein_grf_list) / len(ein_grf_list)
	# Q18
	print "Q18, ave Grf eout", sum(eout_grf_list) / len(eout_grf_list)

# Q16, ave ein 0.0716196666667
# Q17, ave Grf ein 0.0291
# Q18, ave Grf eout 0.07972
